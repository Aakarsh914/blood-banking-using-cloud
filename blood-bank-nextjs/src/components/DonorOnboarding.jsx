...
export default function DonorOnboarding({ onComplete }) {
  ...
  const [quizPassed, setQuizPassed] = useState(user?.role === "RECEIVER");
  ...
  return (
    <>
      {!quizPassed ? (
        <EligibilityQuiz onComplete={() => setQuizPassed(true)} />
      ) : (
        <div className="card" style={{ maxWidth: "600px", margin: "2rem auto", animation: "slideInRight 0.5s ease-out" }}>
      <h2 style={{ textAlign: "center", marginBottom: "8px", color: "var(--danger)" }}>Finalize Your {user?.role === 'RECEIVER' ? 'Receiver' : 'Donor'} Profile</h2>
